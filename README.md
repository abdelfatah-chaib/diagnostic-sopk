...
```
diagnostic-sopk/
├── app.py                         # Application principale (dashboard)
├── Accueil.py                     # Page d'accueil alternative
├── image_manager.py               # Gestion des images avec fallback
├── requirements.txt               # Dépendances Python
├── images/                        # Graphiques et résultats générés
└── pages/
    ├── 1_Pipeline_1_Clinique.py   # Pipeline 1 — ML classique
    ├── 2_Pipeline_2_Logique_Floue.py  # Pipeline 2 — Hybride
    ├── 3_Pipeline_3_Clustering.py # Pipeline 3 — Non supervisé
    ├── 4_Comparaison_Globale.py   # Comparaison finale
    └── 5_A_Propos.py              # À propos du projet
```
...
| Modèle | AUC-ROC | Recall | MCC | Dice Score (F1) |
|--------|---------|--------|-----|-----------------|
| Random Forest (Pipe 3) | **0.93** | 0.84 | 0.80 | 0.85 |
| Algo Génétique + RF (Pipe 2) | 0.83 | 0.72 | 0.34 | 0.72 |
| Borderline-SMOTE + RF (Pipe 1) | 0.66 | 0.38 | 0.37 | 0.50 |

> 🏆 **Meilleur modèle global** : Random Forest (Pipeline 3) avec un AUC-ROC de 0.93 et un F1-Score de 0.85.
